...
async def startup_tasks():
    ...
    # 如果启用了账号池，跳过 Cloudflare 残留 Workers 清理与后台任务
    import os
    pool_base_prefetch = os.getenv("POOL_SERVICE_BASE_URL") or os.getenv("POOL_SERVICE_URL")
    if not pool_base_prefetch:
        try:
            from warp_token_manager import startup_cleanup, init_worker_cleanup_tasks
            await startup_cleanup(prefix="warp-token-")  # 阻塞，清理完成再继续
            init_worker_cleanup_tasks(prefix="warp-token-", threshold=50, interval_seconds=1800)  # 后台周期任务
            logger.info("✅ 启动前清理完成，已启动周期清理任务")
        except Exception as e:
            logger.warning(f"⚠️ 启动前清理遇到问题（继续启动）: {e}")

    import os
    # 如果启用了账号池，跳过 Cloudflare Token 池初始化与匿名路径
    pool_base = os.getenv("POOL_SERVICE_BASE_URL") or os.getenv("POOL_SERVICE_URL")
    if pool_base:
        logger.info("检测到账号池配置，跳过 Cloudflare Token 池初始化与匿名token路径")
    else:
        logger.info("未配置账号池；开发模式下不再自动申请匿名token，仅提示")
        logger.warning("⚠️ 未找到JWT且未配置账号池，建议设置 POOL_SERVICE_BASE_URL 后重试。")
        # 直接跳过旧的 Cloudflare Token 池与匿名token逻辑
        has_cf_config = False
        ...
        if accounts_json:
            try:
                ...
                if isinstance(accounts_data, list) and len(accounts_data) > 0:
                    has_cf_config = True
                    multi_account_count = len(accounts_data)
                    logger.info(f"检测到 JSON 格式的多账号配置，共 {multi_account_count} 个账号")
            except json.JSONDecodeError:
                pass

        # 方式2：编号的环境变量
        if not has_cf_config:
            index = 1
            while True:
            ...
            if multi_account_count > 0:
                has_cf_config = True
                logger.info(f"检测到编号格式的多账号配置，共 {multi_account_count} 个账号")
        ...
        if not has_cf_config:
            ...
            if cf_api_token and cf_account_id:
                has_cf_config = True
                multi_account_count = 1
                logger.info("检测到单账号配置")

        if has_cf_config:
            try:
                if multi_account_count > 1:
                    logger.info(f"启用多账号轮换模式，共 {multi_account_count} 个账号")
                else:
                    logger.info("使用单账号模式")
                ...
                # 强制初始化 Token 池并等待填充完成
                # get_token_pool 会自动检测并使用多账号配置
                pool = await get_token_pool()
                logger.info("✅ Token 池初始化成功")

                # 获取池的统计信息
                stats = pool.get_stats()
                logger.info(f"Token 池状态: {stats}")

                # 验证池中是否有可用 token
                try:
                    test_token = await pool.get_valid_token()
                    logger.info("✅ Token 池验证成功，已有可用 token")

                    # 如果是多账号模式，显示使用的是哪个服务
                    if pool.use_multi_account:
                        logger.info("✅ 多账号轮换服务已启用")
                    else:
                        logger.info("使用单账号服务")

                except Exception as e:
                    logger.error(f"❌ Token 池验证失败: {e}")
                    logger.error("❌ 服务启动失败：无法获取有效 token")
                    raise RuntimeError("Token 池初始化失败，服务无法启动")

            except Exception as e:
                logger.error(f"❌ Token 池初始化失败: {e}")
                logger.error("❌ 服务启动失败：Token 池必须成功初始化")
                raise RuntimeError(f"Token 池初始化失败: {e}")
        else:
            logger.info("未配置 Cloudflare，跳过 Token 池初始化")
    ...
    try:
        ...
        if pool_base:
            import httpx
            health_url = f"{pool_base.rstrip('/')}/api/health"
            async with httpx.AsyncClient(timeout=httpx.Timeout(5.0), trust_env=True, verify=False) as client:
                resp = await client.get(health_url)
                if resp.status_code != 200:
                    raise RuntimeError(f"Pool health check failed: HTTP {resp.status_code}")
            logger.info("✅ 账号池可用，跳过匿名token初始化")
        else:
            ...
            if token and not is_token_expired(token):
                ...
                logger.warning("⚠️ 未找到JWT token，且未配置账号池，尝试申请匿名访问token用于额度初始化…")
                try:
